...
Public Class Commonfunction
    ...
    Public Function UKM2_SchoolprofileUpdate(ByVal strStudentID As String, ByVal strExamYear As String) As String
        ...
        Try
            ...
            strWhere += " WHERE a.StudentID=b.StudentID AND b.SchoolID=c.SchoolID AND a.Studentid='" & strStudentID & "' AND a.ExamYear='" & strExamYear & "'"
            ...
            For i As Integer = 0 To mySourceDataSet.Tables("myaccount").Rows.Count - 1
                If Not IsDBNull(mySourceDataSet.Tables("myaccount").Rows(i).Item("SchoolID")) Then
                ...
                strSQL = "UPDATE UKM2 WITH (UPDLOCK) SET SchoolID='" & strSchoolID & "',SchoolState='" & strSchoolState & "',SchoolCity='" & strSchoolCity & "',SchoolType='" & strSchoolType & "',SchoolPPD='" & strSchoolPPD & "',SchoolLokasi='" & strSchoolLokasi & "' WHERE StudentID='" & strStudentID & "' AND ExamYear='" & strExamYear & "'"
    ...
    Public Function UKM2_StudentprofileUpdate(ByVal strStudentID As String, ByVal strExamYear As String) As String
        Dim strRet As String = "0"
        Dim strSQL As String = ""
        Dim strWhere As String = ""
        Dim strOrderBy As String = ""
        ...
        Try
            ...
            tmpSQL = "SELECT DOB_Year,StudentRace,StudentReligion FROM StudentProfile"
            strWhere += " WHERE StudentID='" & strStudentID & "'"
            strSQL = tmpSQL + strWhere
            ...
            For i As Integer = 0 To mySourceDataSet.Tables("myaccount").Rows.Count - 1
                If Not IsDBNull(mySourceDataSet.Tables("myaccount").Rows(i).Item("DOB_Year")) Then
                    strDOB_Year = mySourceDataSet.Tables("myaccount").Rows(i).Item("DOB_Year")
                Else
                    strDOB_Year = ""
                End If

                If Not IsDBNull(mySourceDataSet.Tables("myaccount").Rows(i).Item("StudentRace")) Then
                    strStudentRace = mySourceDataSet.Tables("myaccount").Rows(i).Item("StudentRace")
                Else
                    strStudentRace = ""
                End If

                If Not IsDBNull(mySourceDataSet.Tables("myaccount").Rows(i).Item("StudentReligion")) Then
                    strStudentReligion = mySourceDataSet.Tables("myaccount").Rows(i).Item("StudentReligion")
                Else
                    strStudentReligion = ""
                End If

                strSQL = "UPDATE UKM2 WITH (UPDLOCK) SET DOB_Year='" & strDOB_Year & "',StudentRace='" & strStudentRace & "',StudentReligion='" & strStudentReligion & "' WHERE StudentID='" & strStudentID & "' AND ExamYear='" & strExamYear & "'"
                strRet = ExecuteSQL(strSQL)
                If Not strRet = "0" Then
                    Return strRet
                End If
    ...
    Function ExecuteSqlTransaction() As String
        ...
        Using connection As New SqlConnection(strconn)
            ...
            command.CommandTimeout = 1200    '5minit. timeout in second
    ...
    Public Function UKM2DONE_CountIQ(ByVal strStudentID As String, ByVal strExamYear As String) As String
        Dim strSQL As String = ""
        Dim strRet As String = ""

        Dim strExamStart As String = ""

        '--ExamStart
        strSQL = "SELECT ExamStart FROM UKM2 WHERE StudentID='" & strStudentID & "' AND ExamYear='" & strExamYear & "'"
        strExamStart = getFieldValue(strSQL)

        Dim strExamStartYear As String = ""
        Dim strExamStartMonth As String = ""

        Dim strDOB_Year As String = "0"
        Dim strDOB_Month As String = "0"
        Dim strNumOfMonths As Integer = 0

        If Not strExamStart.Length = 0 Then
            strExamStartYear = Mid(strExamStart, 1, 4)
            strExamStartMonth = Mid(strExamStart, 5, 2)
        Else
            '--default examdate
            strExamStartYear = Now.Year
            strExamStartMonth = Now.Month
        End If

        strSQL = "SELECT DOB_Year FROM StudentProfile WHERE StudentID='" & strStudentID & "'"
        strDOB_Year = getFieldValue(strSQL)

        strSQL = "SELECT DOB_Month FROM StudentProfile WHERE StudentID='" & strStudentID & "'"
        strDOB_Month = getFieldValue(strSQL)

        '--check isinteger
        If Not strDOB_Year.Length = 0 Then
            Dim dtDOB As New DateTime(strDOB_Year, strDOB_Month, 1)
            Dim dtExam As New DateTime(strExamStartYear, strExamStartMonth, 1)

            strNumOfMonths = MonthDifference(dtDOB, dtExam)
        Else
            strNumOfMonths = 0
        End If

        '--1
        strSQL = "UPDATE UKM2 SET Student_Age_Month=" & strNumOfMonths & " WHERE StudentID='" & strStudentID & "' AND ExamYear='" & strExamYear & "'"
        strRet = ExecuteSQL(strSQL)

        '--2
        strSQL = "UPDATE UKM2 SET Student_Age_Year=Student_Age_Month/12.0 WHERE StudentID='" & strStudentID & "' AND ExamYear='" & strExamYear & "'"
        strRet = ExecuteSQL(strSQL)

        '--3
        strSQL = "UPDATE UKM2 SET Mental_Age_Year=(SELECT Mental_Age_Year FROM master_mental_age_score WHERE master_mental_age_score.TotalScore=UKM2.TotalScore) WHERE StudentID='" & strStudentID & "' AND ExamYear='" & strExamYear & "'"
        strRet = ExecuteSQL(strSQL)

        '--4
        strSQL = "UPDATE UKM2 SET Student_IQ=(Mental_Age_Year/Student_Age_Year) * 100 WHERE StudentID='" & strStudentID & "' AND ExamYear='" & strExamYear & "'"
        strRet = ExecuteSQL(strSQL)
    ...
    Private Function ExportData(ByVal dsTable As DataSet, ByVal strTitle As String) As String
        ...
        Dim strFilename As String = strTitle
    ...
    Public Shared Function getPpcsDate() As String
        Dim ppcsdate As String = ""

        Dim strSQL As String = "SELECT configString FROM master_Config WHERE configCode = 'DefaultPPCSDate'"
        ...
        Try
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                If Not IsDBNull(ds.Tables(0).Rows(0).Item(0).ToString) Then
                    ppcsdate = ds.Tables(0).Rows(0).Item(0).ToString
                Else
                    Return ""
                End If
            End If
    ...
    Public Shared Function getUkm3SessionID() As String
        Dim SessionID As String = ""

        Dim strSQL As String = "SELECT parameter FROM general_config WHERE config = 'currentSession'"
        ...
        Try
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                If Not IsDBNull(ds.Tables(0).Rows(0).Item(0).ToString) Then
                    SessionID = ds.Tables(0).Rows(0).Item(0).ToString
                Else
                    Return ""
                End If
            End If
    ...
    Public Function kppClosed() As Boolean
        Try
            Dim strEndDate As String = oCommon.getFieldValue("SELECT parameter FROM general_config WHERE config = 'kppEndDate'")
            Dim today As String = oCommon.getToday()

            If CInt(today) > CInt(strEndDate) Then
                Return True
            End If

            Dim kppStatus As String = oCommon.getFieldValue("SELECT parameter FROM ukm3.dbo.general_config WHERE config = 'kppStatus'")

            If Not kppStatus = "1" Then
                Return True
            End If
    ...
    Public Function ppcsClosed() As Boolean
        Try
            Dim strEndDate As String = oCommon.getFieldValue("SELECT parameter FROM general_config WHERE config = 'ppcsEndDate'")
            Dim today As String = oCommon.getToday()
            ...
            Dim ppcsStatus As String = oCommon.getFieldValue("SELECT parameter FROM ukm3.dbo.general_config WHERE config = 'ppcsStatus'")

            If Not ppcsStatus = "1" Then
                Return True
            End If
    ...
    Public Function rappcsClosed() As Boolean
        Try
            Dim strEndDate As String = oCommon.getFieldValue("SELECT parameter FROM general_config WHERE config = 'rappcaEndDate'")
            Dim today As String = oCommon.getToday()
            ...
            Dim rappcsStatus As String = oCommon.getFieldValue("SELECT parameter FROM ukm3.dbo.general_config WHERE config = 'rappcsStatus'")

            If Not rappcsStatus = "1" Then
                Return True
            End If
    ...
    Public Function tappcsClosed() As Boolean
        Try
            Dim strEndDate As String = oCommon.getFieldValue("SELECT parameter FROM general_config WHERE config = 'tappcaEndDate'")
            Dim today As String = oCommon.getToday()
            ...
            Dim tappcsStatus As String = oCommon.getFieldValue("SELECT parameter FROM ukm3.dbo.general_config WHERE config = 'tappcsStatus'")

            If Not tappcsStatus = "1" Then
                Return True
            End If
