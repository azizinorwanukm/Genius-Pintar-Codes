...
Partial Public Class laporan_harian_create1
    ...
    Private Function ppcs_eval_update() As Boolean
        ...
        ' ''--update status
        'strSQL = "UPDATE UKM2_RESPONDENT_MARK WITH (UPDLOCK) SET LaporanHarian='" & oCommon.getToday & "' WHERE StudentID='" & Request.QueryString("studentid") & "'"
        'strRet = oCommon.ExecuteSQL(strSQL)

        ' ''--activity log
        'Dim strRespFullname As String
        'strSQL = "SELECT RespFullname FROM ukm1_respondent_mark2011 WHERE StudentID='" & strStudentid & "'"
        'strRespFullname = oCommon.getFieldValue(strSQL)

        'strSQL = "SELECT Class2010 FROM ukm2_respondent_mark WHERE StudentID='" & strStudentid & "'"
        'Dim strClass2010 As String = oCommon.getFieldValue(strSQL)

        'strRet = oCommon.ppcs_activity_insert(Server.HtmlEncode(Request.Cookies("ppcs_myguid").Value), Server.HtmlEncode(Request.Cookies("ppcs_usertype").Value), "Kemaskini Laporan Harian. Student ID: " & strStudentid & " " & strRespFullname & " Kelas:" & strClass2010)
    ...
    Private Function ppcs_eval_insert() As Boolean
        ...
        strSQL = "INSERT INTO ppcs_eval_daily (StudentID,DateCreated,CreateBy,PPCSYear,PPCSDate,ClassCode,ClassID,LastUpdate,Q001,Q002,Q003,Q004,Q005,Q006,Q007,Q008,Q009,Q010,Q001Remarks,Q002Remarks,Q003Remarks,Q004Remarks,Q005Remarks,Q006Remarks,Q007Remarks,Q008Remarks,Q009Remarks,Q010Remarks,TotalMark) Values ('" & Request.QueryString("studentid") & "','" & lblSelectedDate.Text & "','" & Server.HtmlEncode(Request.Cookies("ppcs_myguid").Value) & "','" & calToday.SelectedDate.Year & "','" & Request.QueryString("ppcsdate") & "','" & Server.HtmlEncode(Request.Cookies("ppcs_classcode").Value) & "'," & Request.QueryString("classid") & ",'" & oCommon.getNow & "'," & nQ1 & "," & nQ2 & "," & nQ3 & "," & nQ4 & "," & nQ5 & "," & nQ6 & "," & nQ7 & "," & nQ8 & "," & nQ9 & "," & nQ10 & ",'" & oCommon.FixSingleQuotes(Q001Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q002Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q003Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q004Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q005Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q006Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q007Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q008Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q009Remarks.Text) & "','" & oCommon.FixSingleQuotes(Q010Remarks.Text) & "'," & nTotalMark & ")"
        strRet = oCommon.ExecuteSQL(strSQL)
        If strRet = "0" Then
            lblMsgTop.Text = "Rekod berjaya dikemaskini."
            lblMsg.Text = "Rekod berjaya dikemaskini."
        ...
        ''--update status
        'strSQL = "UPDATE UKM2_RESPONDENT_MARK WITH (UPDLOCK) SET LaporanHarian='" & lblSelectedDate.Text & "' WHERE StudentID='" & strStudentid & "'"
        'strRet = oCommon.ExecuteSQL(strSQL)
        ...
        'strRet = oCommon.ppcs_activity_insert(Server.HtmlEncode(Request.Cookies("ppcs_myguid").Value), Server.HtmlEncode(Request.Cookies("ppcs_usertype").Value), "Rekod baru Laporan Harian. Student ID: " & strStudentid & " " & strRespFullname & " Kelas:" & strClass2010)
    ...
    ''Private Sub calToday_VisibleMonthChanged(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.MonthChangedEventArgs) Handles calToday.VisibleMonthChanged
    ''    lblSelectedDate.Text = calToday.SelectedDate.ToString("yyyyMMdd")

    ''    '--load answers
    ''    ppcs_eval_load(strStudentid)
