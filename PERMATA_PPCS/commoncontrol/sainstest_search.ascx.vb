...
Public Class sainstest_search
    ...
    Private Function getSQL() As String
        ...
        tmpSQL = "SELECT SainsTest.SainsTestID,SainsTest.LastUpdate,SainsTest.Fullname,SainsTest.EmailAdd,StudentProfile.StudentFullname,StudentProfile.StudentEmail,StudentProfile.AlumniID,SainsTest.SurveyID,SainsTest.LastPage,SainsTest.IsCompleted,SainsTest.ScorePercentage FROM SainsTest"
        tmpSQL += " LEFT OUTER JOIN StudentProfile ON SainsTest.StudentID=StudentProfile.StudentID"
        strWhere = " WHERE IsDeleted='N'"


        '--PPCSDate
        If Not ddlPPCSDate.Text = "ALL" Then
            strWhere += " AND SainsTest.PPCSDate='" & ddlPPCSDate.Text & "'"
        End If

        '--IsCompleted
        If Not selIsCompleted.Value.Length = 0 Then
            strWhere += " AND SainsTest.IsCompleted='" & selIsCompleted.Value & "'"
        End If
        ...
        If Not ddlSurveyID.Text = "ALL" Then
            strWhere += " AND SainsTest.SurveyID='" & ddlSurveyID.Text & "'"
        End If

        If selOrderBy.Value = "1" Then
            strOrder = " ORDER BY SainsTest.LastUpdate DESC"
        Else
            strOrder = " ORDER BY SainsTest.ScorePercentage DESC"
        End If
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        For i As Integer = 0 To datRespondent.Rows.Count - 1
            ...
            If isChecked Then
                strSQL = "UPDATE SainsTest SET IsDeleted='Y' WHERE SainsTestID=" & datRespondent.DataKeys(i).Value.ToString
    ...
    Private Sub datRespondent_SelectedIndexChanging(sender As Object, e As GridViewSelectEventArgs) Handles datRespondent.SelectedIndexChanging
        ...
        strSQL = "SELECT LoginID FROM SainsTest WHERE SainsTestID=" & strKeyID
        Dim strLoginID As String = oCommon.getFieldValue(strSQL)

        strSQL = "SELECT SurveyID FROM SainsTest WHERE SainsTestID=" & strKeyID
        Dim strSurveyID As String = oCommon.getFieldValue(strSQL)

        Dim strLink As String = oCommon.getAppsettings("DomainSainsTest") & "home.candidate.aspx?culture=en-US&surveyid=" & strSurveyID & "&loginid=" & strLoginID
    ...
    Protected Sub btnIsComplete_Click(sender As Object, e As EventArgs) Handles btnIsComplete.Click
        For i As Integer = 0 To datRespondent.Rows.Count - 1
            ...
            If isChecked Then
                strSQL = "UPDATE SainsTest SET IsCompleted='" & selSetIsCompleted.Value & "' WHERE SainsTestID=" & datRespondent.DataKeys(i).Value.ToString
                oCommon.ExecuteSQL(strSQL)
            Else
                '--do nothing
            End If
