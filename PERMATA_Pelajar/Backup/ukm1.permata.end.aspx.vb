...
Partial Public Class ukm1_permata_end
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                '--set DONE updated by batch file run END-OF-TEST
                'setUKM1DONE()
                ...
                UKM1Result.Text = ConfigurationManager.AppSettings("UKM1Result")

                lblLevel.Text = ""    '--not implement in 2013
                '--lblLevel.Text = getLevel()
            End If
    ...
    Private Function getDONE() As Boolean
        Dim strTotalPercentage As String = ""

        '--DONE base on TotalPercentage calculated. Either by students or system back-end
        strSQL = "SELECT TotalPercentage FROM UKM1 WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        strTotalPercentage = oCommon.getFieldValue(strSQL)
        If strTotalPercentage.Length = 0 Then
            lblMsg.Text = "Anda belum menamatkan Ujian UKM1. Sijil penyertaan tidak dapat dijana!"
            Return False
        End If
    ...
    Private Sub getDuration()
        ...
        strSQL = "SELECT ExamStart FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
    ...
    Private Function getTotalscore() As String
        strSQL = "SELECT TotalScore FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
    ...
    Private Sub setUKM1DONE()
        ''--update once only
        strSQL = "SELECT Status FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        If oCommon.getFieldValue(strSQL) = "DONE" Then
            Exit Sub
        End If

        ''--update UKM1
        strSQL = "UPDATE UKM1 WITH (UPDLOCK) SET Status='DONE' WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        ...
        strSQL = "UPDATE UKM1 WITH (UPDLOCK) SET ModA=(SELECT SUM(isnull(Q001,0)+isnull(Q002,0)+isnull(Q003,0)+isnull(Q004,0)+isnull(Q005,0)+isnull(Q006,0)+isnull(Q007,0)+isnull(Q008,0)+isnull(Q009,0)+isnull(Q010,0)+isnull(Q011,0)+isnull(Q012,0)+isnull(Q013,0)+isnull(Q014,0)+isnull(Q015,0)+isnull(Q016,0)+isnull(Q017,0)+isnull(Q018,0)+isnull(Q019,0)+isnull(Q020,0)+isnull(Q021,0)+isnull(Q022,0)+isnull(Q023,0)+isnull(Q024,0)+isnull(Q025,0)+isnull(Q026,0)+isnull(Q027,0)+isnull(Q028,0)+isnull(Q029,0)+isnull(Q030,0)+isnull(Q031,0)+isnull(Q032,0)+isnull(Q033,0)+isnull(Q034,0)+isnull(Q035,0)+isnull(Q036,0)+isnull(Q037,0)+isnull(Q038,0)+isnull(Q039,0)+isnull(Q040,0)+isnull(Q041,0)+isnull(Q042,0)+isnull(Q043,0)+isnull(Q044,0)+isnull(Q045,0)+isnull(Q046,0)+isnull(Q047,0)+isnull(Q048,0)+isnull(Q049,0)+isnull(Q050,0)+isnull(Q051,0)+isnull(Q052,0)+isnull(Q053,0)+isnull(Q054,0)+isnull(Q055,0)+isnull(Q056,0)+isnull(Q057,0)+isnull(Q058,0)+isnull(Q059,0)+isnull(Q060,0)) as ModA FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "') WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        oCommon.ExecuteSQL(strSQL)
        ''-debug 
        'Response.Write("ModA:" & strSQL)

        ''--SUM here ModB. TOTAL=30. Soalan 61-90
        strSQL = "UPDATE UKM1 WITH (UPDLOCK) SET ModB=(SELECT SUM(isnull(Q061,0)+isnull(Q062,0)+isnull(Q063,0)+isnull(Q064,0)+isnull(Q065,0)+isnull(Q066,0)+isnull(Q067,0)+isnull(Q068,0)+isnull(Q069,0)+isnull(Q070,0)+isnull(Q071,0)+isnull(Q072,0)+isnull(Q073,0)+isnull(Q074,0)+isnull(Q075,0)+isnull(Q076,0)+isnull(Q077,0)+isnull(Q078,0)+isnull(Q079,0)+isnull(Q080,0)+isnull(Q081,0)+isnull(Q082,0)+isnull(Q083,0)+isnull(Q084,0)+isnull(Q085,0)+isnull(Q086,0)+isnull(Q087,0)+isnull(Q088,0)+isnull(Q089,0)+isnull(Q090,0)) as ModB FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "') WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        oCommon.ExecuteSQL(strSQL)
        'Response.Write("ModB:" & strSQL)

        ''--SUM here ModC. TOTAL=30. Soalan 105-134
        strSQL = "UPDATE UKM1 WITH (UPDLOCK) SET ModC=(SELECT SUM(isnull(Q105,0)+isnull(Q106,0)+isnull(Q107,0)+isnull(Q108,0)+isnull(Q109,0)+isnull(Q110,0)+isnull(Q111,0)+isnull(Q112,0)+isnull(Q113,0)+isnull(Q114,0)+isnull(Q115,0)+isnull(Q116,0)+isnull(Q117,0)+isnull(Q118,0)+isnull(Q119,0)+isnull(Q120,0)+isnull(Q121,0)+isnull(Q122,0)+isnull(Q123,0)+isnull(Q124,0)+isnull(Q125,0)+isnull(Q126,0)+isnull(Q127,0)+isnull(Q128,0)+isnull(Q129,0)+isnull(Q130,0)+isnull(Q131,0)+isnull(Q132,0)+isnull(Q133,0)+isnull(Q134,0)) as ModC FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "') WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        oCommon.ExecuteSQL(strSQL)
        'Response.Write("ModC:" & strSQL)

        ''--SUM TotalScore
        strSQL = "UPDATE UKM1 WITH (UPDLOCK) SET TotalScore=(SELECT sum(ModA+ModB+ModC) as TotalScore FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "') WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        oCommon.ExecuteSQL(strSQL)
        'Response.Write("TotalScore:" & strSQL)

        ''--SUM TotalPercentage
        strSQL = "UPDATE UKM1 WITH (UPDLOCK) SET TotalPercentage=(SELECT CONVERT(DECIMAL(5,2),100.0 * (SELECT TotalScore FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "') / " & ConfigurationManager.AppSettings("FullMark") & ") FROM UKM1  WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "') WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        oCommon.ExecuteSQL(strSQL)
    ...
    Private Sub btnCreate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreate.Click
        ...
        If CInt(lblTotalMin.Text) < CInt(strMinDuration) Then
            lblMsg.Text = "Sijil hanya akan dijana untuk pelajar yang menghabiskan ujian melebihi " & strMinDuration & " minit sahaja. Harap Maklum."
            Exit Sub
        End If

        '--limit TotalPercentage <20%
        Dim strMinMark As String = ConfigurationManager.AppSettings("MinMark")
        strSQL = "SELECT TotalPercentage FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & Request.QueryString("examyear") & "'"
        Dim strTotalPercentage As String = oCommon.getFieldValueInt(strSQL)
        'Response.Write("strTotalPercentage:" & strTotalPercentage)

        '--menamatkan ujian ukm1 atau tunggu END-OF-TEST process
        If CInt(strTotalPercentage) < CInt(strMinMark) Then
            lblMsg.Text = "Sijil hanya akan dijana untuk pelajar yang komited dalam menjawab Ujian UKM1 sahaja. Harap Maklum."
            Exit Sub
        End If
        ...
        Try
            ...
            msFileName = Request.QueryString("studentid") & "-UKM1-" & lblExamYear.Text & ".pdf"
