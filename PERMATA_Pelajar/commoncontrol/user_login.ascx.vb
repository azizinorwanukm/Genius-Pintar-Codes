...
Partial Public Class user_login
    ...
    Private Sub btnLogin_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLogin.Click
        ...
        strSQL = "Select Pwd FROM StudentProfile WITH (NOLOCK) WHERE LoginID='" & oCommon.FixSingleQuotes(txtLoginID.Text) & "'"
        strPwd = oCommon.getFieldValue(strSQL)

        If strPwd.Length = 0 Then
            ''--first time user
            strSQL = "Select LoginID FROM StudentProfile WITH (NOLOCK) WHERE LoginID='" & oCommon.FixSingleQuotes(txtLoginID.Text) & "'"
        Else
            ''--existing user and change password done
            strSQL = "Select LoginID FROM StudentProfile WITH (NOLOCK) WHERE LoginID='" & oCommon.FixSingleQuotes(txtLoginID.Text) & "' AND Pwd='" & oCommon.FixSingleQuotes(txtPwd.Text) & "'"
        End If
        ...
        Else
            lblMsg.Text = "MYKAD/MYKID/Surat Beranak tidak terdapat di dalam pengkalan data kami."
        End If
