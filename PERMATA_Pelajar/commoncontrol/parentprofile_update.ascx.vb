...
Partial Public Class Parentprofile_update
    ...
    Private Sub LoadPage()
        strSQL = "SELECT * FROM ParentProfile Where StudentID='" & CType(Session.Item("permata_studentid"), String) & "'"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
    ...
    Private Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        Try
            ...
            strSQL = "SELECT ParentID FROM ParentProfile WHERE StudentID='" & CType(Session.Item("permata_studentid"), String) & "'"
            If oCommon.isExist(strSQL) = True Then
                ''--update only
                strRet = Parentprofile_update()
            Else
                ''parentprofile not created during creation. not mandatory anymore
                strRet = ParentProfile_insert()
            End If
    ...
    Private Function Parentprofile_update() As String
        ...
        ''log
        oCommon.TransactionLog("parentprofile_update", oCommon.FixSingleQuotes(strSQL), Request.UserHostAddress, CType(Session.Item("permata_mykad"), String))
