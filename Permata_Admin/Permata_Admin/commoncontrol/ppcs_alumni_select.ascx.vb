    ...
    Private Function setLayak(ByVal strStudentID As String) As Boolean
        ...
        Using connection As New SqlConnection(strconn)
            ...
            Try
                ...
                If oCommon.isExist(strSQL) = False Then
                    '--INSERT
                    Dim examyear_id As String = oCommon.getFieldValue("select master_examyear.examyearid from master_examyear join master_Config on master_Examyear.ExamYear = master_Config.configString where master_Config.configCode='DefaultExamYear'")

                    If oCommon.getAppsettings("DefaultExamYear") = getUKM1Year Then
                        strSQL = "INSERT INTO UKM1_" & getUKM1Year & " (StudentID,ExamYear,QuestionYear,HostAddress,HostName,Browser,SelectedLang,Status,DOB_Year,examyear_id) VALUES('" & strStudentID & "','" & oCommon.getAppsettings("DefaultExamYear") & "','" & oCommon.getAppsettings("DefaultExamYear") & "','" & Request.UserHostAddress & "','" & Request.UserHostName & "','" & Request.UserAgent & "','ms-MY','NEW','" & strDOB_Year & "','" & examyear_id & "')"
                        command.CommandText = strSQL
                        command.ExecuteNonQuery()
                    End If

                    strSQL = "INSERT INTO UKM1 (StudentID,ExamYear,QuestionYear,HostAddress,HostName,Browser,SelectedLang,Status,DOB_Year,examyear_id) VALUES('" & strStudentID & "','" & oCommon.getAppsettings("DefaultExamYear") & "','" & oCommon.getAppsettings("DefaultExamYear") & "','" & Request.UserHostAddress & "','" & Request.UserHostName & "','" & Request.UserAgent & "','ms-MY','NEW','" & strDOB_Year & "','" & examyear_id & "')"
                    command.CommandText = strSQL
                    command.ExecuteNonQuery()
