...
Partial Public Class ukm2_schoolprofile_list1
    ...
    Private Function BindData(ByVal gvTable As GridView) As Boolean
        ...
        If Not selisHadir.Value = "ALL" Then
            strWhereIn += " AND UKM2.IsHadir='" & selisHadir.Value & "'"
        End If

        tmpSQL = "select SUM(Jumlah) totalStudent
                  FROM(SELECT COUNT(*) as Jumlah FROM UKM2 LEFT OUTER JOIN StudentSchool ON UKM2.StudentID=StudentSchool.StudentID AND StudentSchool.IsLatest='Y'
                  LEFT OUTER JOIN SchoolProfile on SchoolProfile.SchoolID=StudentSchool.SchoolID"
        strWhere = " where UKM2.Examyear='" & ddlExamYear.Text & "'" & strWhereIn & ""
    ...
    Private Function getSQL() As String
        ...
        tmpSQL = "SELECT SchoolProfile.SchoolID,SchoolProfile.SchoolState,SchoolProfile.SchoolPPD,SchoolProfile.SchoolCity,SchoolProfile.SchoolCode,SchoolProfile.SchoolName,SchoolProfile.SchoolLokasi,"
        '--remove left outer join. get the schoolid when student take the exam
        tmpSQL += "(SELECT COUNT(*) FROM UKM2 LEFT OUTER JOIN StudentSchool ON UKM2.StudentID=StudentSchool.StudentID AND StudentSchool.IsLatest='Y' WHERE StudentSchool.SchoolID=SchoolProfile.SchoolID AND UKM2.Examyear='" & ddlExamYear.Text & "'" & strWhereIn & ") as Jumlah FROM SchoolProfile"
        'tmpSQL += "(SELECT COUNT(*) FROM UKM2 WHERE UKM2.SchoolID=SchoolProfile.SchoolID AND UKM2.Examyear='" & ddlExamYear.Text & "'" & strWhereIn & ") as Jumlah FROM SchoolProfile"
    ...
    Private Sub datRespondent_SelectedIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSelectEventArgs) Handles datRespondent.SelectedIndexChanging
        ...
        Try
            Select Case getUserProfile_UserType()
                Case "ADMIN"
                    Response.Redirect("ukm2.schoolprofile.student.list.aspx?schoolid=" & strKeyID & "&examyear=" & ddlExamYear.Text & "&ishadir=" & selisHadir.Value)
                Case "ADMINOP"
                    Response.Redirect("ukm2.schoolprofile.student.list.aspx?schoolid=" & strKeyID & "&examyear=" & ddlExamYear.Text & "&ishadir=" & selisHadir.Value)
                Case Else
