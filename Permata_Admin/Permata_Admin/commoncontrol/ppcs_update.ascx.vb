...
Partial Public Class ppcs_update
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                '--get PPCSDate
                strSQL = "SELECT PPCSDate FROM PPCS WHERE PPCSID='" & Request.QueryString("PPCSID") & "'"
                lblPPCSDate.Text = oCommon.getFieldValue(strSQL)
                PPCS_Course_list()

                '--get CourseID
                strSQL = "SELECT CourseID FROM PPCS WHERE PPCSID='" & Request.QueryString("PPCSID") & "'"
                lblCourseID.Text = oCommon.getFieldValue(strSQL)
    ...
    Private Sub PPCS_Course_list()
        ...
        Try
            ...
            '--add blank course for TOLAK
            ddlPPCSCourse.Items.Add(New ListItem("NULL", ""))
    ...
    Private Sub PPCS_Class_list()
        strSQL = "SELECT ClassID,ClassCode FROM PPCS_Class WHERE CourseID=" & lblCourseID.Text & " ORDER BY ClassCode"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
    ...
    Private Function ppcs_update() As Boolean
        ...
        '--course null, class automatic null
        If lblCourseID.Text.Length = 0 Then
            strSQL = "UPDATE PPCS WITH (UPDLOCK) SET CourseID=NULL,PPCSCourse=NULL,ClassID=NULL,PPCSClass=NULL,Tempat='" & oCommon.FixSingleQuotes(lblTempat.Text.ToUpper) & "',NamaAsrama='" & oCommon.FixSingleQuotes(txtNamaAsrama.Text) & "',NoBilik='" & oCommon.FixSingleQuotes(txtNoBilik.Text) & "',SaizBaju='" & oCommon.FixSingleQuotes(txtSaizBaju.Text) & "',SakitAlahan='" & oCommon.FixSingleQuotes(txtSakitAlahan.Text) & "' WHERE PPCSID='" & Request.QueryString("PPCSID") & "'"
        End If

        '--course exisit. class nill. update course only
        If lblCourseID.Text.Length > 0 And lblClassID.Text.Length = 0 Then
            strSQL = "UPDATE PPCS WITH (UPDLOCK) SET CourseID=" & ddlPPCSCourse.SelectedValue & ",PPCSCourse='" & ddlPPCSCourse.SelectedItem.Text & "',ClassID=NULL,PPCSClass=NULL,Tempat='" & oCommon.FixSingleQuotes(lblTempat.Text.ToUpper) & "',NamaAsrama='" & oCommon.FixSingleQuotes(txtNamaAsrama.Text) & "',NoBilik='" & oCommon.FixSingleQuotes(txtNoBilik.Text) & "',SaizBaju='" & oCommon.FixSingleQuotes(txtSaizBaju.Text) & "',SakitAlahan='" & oCommon.FixSingleQuotes(txtSakitAlahan.Text) & "' WHERE PPCSID='" & Request.QueryString("PPCSID") & "'"
        End If
    ...
    Private Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        '    lblMsg.Text = "Sila pilih KELAS terlebih dahulu. Jika tiada, masukkan terlebih dahulu melalui Sistem PPCS."
        '    Exit Sub
        'End If
    ...
    Private Sub ddlPPCSCourse_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPPCSCourse.TextChanged
        Try
            ...
            '--set default class base on selected course
            ddlPPCSClass.SelectedIndex = 0
