...
Partial Public Class ukm2_select
    ...
    Private Function getSQL() As String
        ...
        Dim strOrder As String = " Order By e.SchoolCode,e.Schoolname ASC"

        tmpSQL = "SELECT a.StudentID,c.MYKAD,c.StudentFullname,c.DOB_Year,d.SchoolID,e.SchoolCode,e.Schoolname,e.Schoolcity,e.SchoolPPD,a.IsHadir,a.IsLogin,a.ExamYear,a.SessiUKM2,a.TarikhUjian,b.FamilyContactNo,b.FatherFullname,(SELECT PusatName FROM PusatUjian WHERE PusatCode=a.PusatCode) as PusatName"
        tmpSQL += " FROM UKM2 a, ParentProfile b, StudentProfile c, StudentSchool d, SchoolProfile e"
        strWhere += " WITH (NOLOCK) WHERE a.StudentID=b.StudentID AND a.StudentID=c.StudentID AND a.StudentID=d.StudentID AND d.SchoolID=e.SchoolID"

        ''ddlExamYear
        If Not ddlExamYear.Text = "ALL" Then
            strWhere += " AND a.ExamYear='" & oCommon.FixSingleQuotes(ddlExamYear.Text) & "'"
        End If

        ''DOB_Year
        If Not ddlDOB_Year.Text = "ALL" Then
            strWhere += " AND c.DOB_Year='" & ddlDOB_Year.Text & "'"
        End If

        ''negeri
        If Not ddlSchoolState.Text = "ALL" Then
            strWhere += " AND e.SchoolState='" & ddlSchoolState.Text & "'"
        End If

        ''ppd
        If Not ddlSchoolPPD.Text = "ALL" Then
            strWhere += " AND e.SchoolPPD='" & ddlSchoolPPD.Text & "'"
        End If
        ...
        If getUserProfile_UserType() = "MRSM" Then
            strWhere += " AND e.SchoolType='MRSM'"
        End If
    ...
    Private Function getSQLExport() As String
        Dim tmpSQL As String
        Dim strWhere As String = ""
        Dim strOrder As String = " Order By TarikhUjian,SessiUKM2 DESC"
        ...
        '--isHadir
        If Not selisHadir.Value = "ALL" Then
            strWhere += " AND IsHadir='" & selisHadir.Value & "'"
        End If
    ...
    Private Function getPusatName(ByVal strStudentID As String) As String
        ...
        strSQL = "SELECT PusatName FROM PusatUjian WHERE PusatCode='" & strPusatCode & "'"
        strRet = oCommon.getFieldValue(strSQL)
    ...
    Private Sub datRespondent_SelectedIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSelectEventArgs) Handles datRespondent.SelectedIndexChanging
        ...
        Try
            Select Case getUserProfile_UserType()
                Case "ADMIN"
                    Response.Redirect("studentprofile.view.aspx?studentid=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case "SUBADMIN"
                    Response.Redirect("subadmin.studentprofile.view.aspx?studentid=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case Else
                    lblMsg.Text = "Invalid user type! No access right."
            End Select
        Catch ex As Exception
