...
Partial Public Class pusatujian_list_kehadiran
    ...
    Private Sub SchoolState_list()
        ...
        Try
            ...
            ddlPusatState.DataTextField = "schoolstate"
            ddlPusatState.DataValueField = "schoolstate"
    ...
    Private Function getSQL() As String
        ...
        Dim strOrder As String = " ORDER BY PusatState,PusatCity,PusatName"

        tmpSQL = "SELECT * FROM PusatUjian"
        strWhere = " WITH (NOLOCK) WHERE PusatTahun='" & ddlExamYear.Text & "'"

        ''state
        If Not ddlPusatState.Text = "ALL" Then
            strWhere += " AND PusatState='" & ddlPusatState.Text & "'"
        End If

        ''city
        If Not ddlPusatPPD.Text = "ALL" Then
            strWhere += " AND PusatPPD='" & ddlPusatPPD.Text & "'"
        End If
    ...
    Private Sub ddlSchoolState_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPusatState.TextChanged
        pusatujian_ppd_list()
    ...
    Private Sub datRespondent_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles datRespondent.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim lblJumPelajar As Label

            Dim i As Integer = e.Row.RowIndex + 1
            Dim strKeyID As String = datRespondent.DataKeys(e.Row.RowIndex).Value.ToString

            lblJumPelajar = e.Row.FindControl("lblJumPelajar")
            lblJumPelajar.Text = getTotalStudents(strKeyID)

        End If

    End Sub

    Private Function getTotalStudents(ByVal strKeyID As String)
        strSQL = "SELECT COUNT(*) FROM UKM2 WHERE PusatCode='" & strKeyID & "' AND ExamYear='" & ddlExamYear.Text & "'"
    ...
    Private Sub datRespondent_SelectedIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSelectEventArgs) Handles datRespondent.SelectedIndexChanging
        ...
        Try
            Select Case getUserProfile_UserType()
                Case "ADMIN"
                    Response.Redirect("admin.pusatujian.kehadiran.pelajar.aspx?pusatcode=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case "SUBADMIN"
                    Response.Redirect("subadmin.pusatujian.kehadiran.pelajar.aspx?pusatcode=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case "UKM"
                    Response.Redirect("ukm.pusatujian.kehadiran.pelajar.aspx?pusatcode=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case "JPN"
                    Response.Redirect("jpn.pusatujian.kehadiran.pelajar.aspx?pusatcode=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case "KPM"
                    Response.Redirect("kpm.pusatujian.kehadiran.pelajar.aspx?pusatcode=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case "KPT"
                    Response.Redirect("kpt.pusatujian.kehadiran.pelajar.aspx?pusatcode=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case "MRSM"
                    Response.Redirect("mara.pusatujian.kehadiran.pelajar.aspx?pusatcode=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case "ASASI"
                    Response.Redirect("asasi.pusatujian.kehadiran.pelajar.aspx?pusatcode=" & strKeyID & "&examyear=" & ddlExamYear.Text)
                Case Else
