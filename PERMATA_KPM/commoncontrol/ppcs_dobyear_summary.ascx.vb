...
Public Class ppcs_dobyear_summary
    ...
    Private Sub dat_Age_SelectedIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSelectEventArgs) Handles dat_Age.SelectedIndexChanging
        ...
        Try
            Select Case getUserProfile_UserType()
                Case "ADMIN"
                    Response.Redirect("admin.ppcs.dobyear.list.aspx?ppcsdate=" & ddlPPCSDate.Text & "&studentrace=" & ddlStudentRace.Text & "&dob_year=" & Server.UrlEncode(strKeyID))
                Case "SUBADMIN"
                    Response.Redirect("subadmin.ppcs.dobyear.list.aspx?ppcsdate=" & ddlPPCSDate.Text & "&studentrace=" & ddlStudentRace.Text & "&dob_year=" & Server.UrlEncode(strKeyID))
                Case Else
            End Select
        Catch ex As Exception
        End Try
    ...
    Private Function getSQL01() As String
        ...
        Dim tmpSQL As String = "SELECT DISTINCT b.DOB_Year,YEAR(getdate()) - b.DOB_Year as StudentAge,COUNT(*) AS Jumlah FROM PPCS a, StudentProfile b"
