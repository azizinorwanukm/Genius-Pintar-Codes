...
Public Class instruktor_list
    ...
    Private Function getSQL() As String
        ...
        If Not txtNoPelajar.Text.Length = 0 Then
            strWhere += " AND koko_instruktor.NoPelajar='" & oCommon.FixSingleQuotes(txtNoPelajar.Text) & "'"
        End If
            ...
            Select Case Server.HtmlEncode(Request.Cookies("koko_usertype").Value)
                Case "ADMIN"
                    Select Case Request.QueryString("set")
                        Case "koko"
                            Response.Redirect("admin.koko.instruktor.assign.aspx?instruktorid=" & strKeyID & "&tahun=" & ddlTahun.Text)
                        Case "kehadiran"
                            Response.Redirect("admin.event.list.aspx?instruktorid=" & strKeyID & "&tahun=" & ddlTahun.Text)
                        Case "markah"
                            Response.Redirect("admin.instruktor.mark.aspx?instruktorid=" & strKeyID & "&tahun=" & ddlTahun.Text)
                        Case Else
                            Response.Redirect("admin.instruktor.view.aspx?instruktorid=" & strKeyID & "&tahun=" & ddlTahun.Text)
                    End Select
    ...
    Protected Sub btnCreate_Click(sender As Object, e As EventArgs) Handles btnCreate.Click
        Response.Redirect("admin.instruktor.create.aspx?tahun=" & ddlTahun.Text)
