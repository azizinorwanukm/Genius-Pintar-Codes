...
Public Class Commonfunction
    ...
    Public Function set_pelajar_Gred(ByVal strTahun As String, ByVal strPeperiksaan As String) As String
        ...
        '--update GredP1
        strSQL = "UPDATE koko_pelajar SET Gred" & strPeperiksaan & "=(SELECT Gred FROM koko_gred WHERE koko_gred.Markah=koko_pelajar.Markah" & strPeperiksaan & " AND Tahun='" & strTahun & "')"
        strSQL += " WHERE Tahun='" & strTahun & "'"
        strRet = ExecuteSQL(strSQL)
        If Not strRet = "0" Then
            Return strRet
        ...
        'PNG
        strSQL = "UPDATE koko_pelajar SET PNG" & strPeperiksaan & "=(SELECT PNG FROM koko_gred WHERE koko_gred.Markah=koko_pelajar.Markah" & strPeperiksaan & " AND Tahun='" & strTahun & "')"
        strSQL += " WHERE Tahun='" & strTahun & "'"
        strRet = ExecuteSQL(strSQL)
        If Not strRet = "0" Then
            Return strRet
    ...
    Function StringCat(ByVal stringToTruncate As String, ByVal length As Integer) As String
        Return If(stringToTruncate.Length > length, stringToTruncate.Substring(0, length) & " ...", stringToTruncate)
