...
Public Class koko_select_sukan
    ...
    Private Function BindDataSQL(ByVal gvTable As GridView) As Boolean
        ...
        Dim tmpSQL As String = "SELECT *,(SELECT COUNT(*) FROM koko_pelajar WHERE koko_pelajar.Tahun='" & ddlTahun.Text & "' AND koko_pelajar.SukanID=koko_kolejpermata.KokoID) as JumlahPelajar FROM koko_kolejpermata"
    ...
    Private Sub datSukan_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles datSukan.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim i As Integer = e.Row.RowIndex + 1
            Dim strKeyID As String = datSukan.DataKeys(e.Row.RowIndex).Value.ToString  'KelasID

            '--instruktor list
            Dim lblInstruktor As Label
            lblInstruktor = e.Row.FindControl("lblInstruktor")
            strSQL = "SELECT Fullname FROM koko_instruktor WHERE SukanID=" & strKeyID & " AND Tahun='" & ddlTahun.Text & "' ORDER BY Fullname"
            ...
            strSQL = "SELECT Fullname FROM koko_instruktor WHERE SukanID=" & strKeyID & " AND KetuaSukan='True' AND Tahun='" & ddlTahun.Text & "' ORDER BY Fullname"
            lblKetuaInstruktor.Text = oCommon.getRowValue(strSQL)
            ...
            strSQL = "SELECT ContactNo FROM koko_instruktor WHERE SukanID=" & strKeyID & " AND KetuaSukan='True' AND Tahun='" & ddlTahun.Text & "' ORDER BY Fullname"
    ...
    Private Sub datSukan_SelectedIndexChanging(sender As Object, e As GridViewSelectEventArgs) Handles datSukan.SelectedIndexChanging
        Dim strKeyID As String = datSukan.DataKeys(e.NewSelectedIndex).Value.ToString
            ...
            Select Case Server.HtmlEncode(Request.Cookies("kokoadmin_usertype").Value)
                Case "ADMIN"
                    Select Case Request.QueryString("set")
                        ...
                        Case Else
                            Response.Redirect("admin.koko.kelaskoko.list.aspx?kokoid=" & strKeyID & "&admin_ID=" & Request.QueryString("admin_ID"))
                    End Select
