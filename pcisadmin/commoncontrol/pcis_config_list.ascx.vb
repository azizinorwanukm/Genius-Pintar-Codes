...
Public Class pcis_config_list
    ...
    Private Function getSQL() As String
        ...
        Dim strOrder As String = " ORDER BY configCode ASC"

        Try
            tmpSQL = "SELECT * FROM pcis_config"
            strWhere += " WHERE pcisconfigid IS NOT NULL"

            '--configCode
            If Not txtconfigCode.Text.Length = 0 Then
                strWhere += " AND configCode LIKE '%" & oCommon.FixSingleQuotes(txtconfigCode.Text.Trim) & "%'"
            End If
    ...
    Private Sub datRespondent_SelectedIndexChanging(sender As Object, e As GridViewSelectEventArgs) Handles datRespondent.SelectedIndexChanging
        ...
        Select Case get_pcis_admin_UserType()
            Case "ADMIN"
                Response.Redirect("admin.pcis.config.update.aspx?pcisconfigid=" & strKeyID)
            Case "SUBADMIN"
            Case "KPT"
            Case "ASASI"
            Case Else
    ...
    Protected Sub btnCreate_Click(sender As Object, e As EventArgs) Handles btnCreate.Click
        Response.Redirect("admin.pcis.config.create.aspx")
    End Sub
End Class