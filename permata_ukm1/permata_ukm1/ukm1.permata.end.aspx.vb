...
Partial Public Class ukm1_permata_end
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                UKM1Result.Text = ConfigurationManager.AppSettings("UKM1DisplayResult")

                '--check kelayakan
                If isLayakSijil() = True Then

                    ''02052019 update UKM1 set flag isLayakSijil = Y if student layak dapat sijil

                    strSQL = "UPDATE UKM1 SET isLayakSijil = 'Y' WHERE StudentID = '" & Request.QueryString("studentid") & "' AND examyear_id = '" & Common.getExamYearID(Now.Year) & "'"

                    Dim ukm1Table As String = Common.getUKM1Table(oCommon.getAppsettings("UKM1ExamYear"))

                    strSQL = "UPDATE " & ukm1Table & " SET isLayakSijil = 'Y' WHERE StudentID = '" & Request.QueryString("studentid") & "' AND examyear_id = '" & Common.getExamYearID(Now.Year) & "'"
    ...
    Private Function isLayakSijil() As Boolean
        ...
        Dim strMinDuration As String = ConfigurationManager.AppSettings("MinDuration")
        'If CInt(lblTotalMin.Text) < CInt(strMinDuration) Then
        '    Return False
        'End If

        '-- check age for print sijil >ROS 11/1/18<
        strSQL = "SELECT DOB_Year FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & oCommon.getAppsettings("UKM1ExamYear") & "'"
        Dim strStdAge As String = oCommon.getFieldValueInt(strSQL)

        Dim Age As Integer = (Now.Year - CInt(strStdAge))

        'get minimum limit for each age

        strSQL = "SELECT TotalPercentage FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & oCommon.getAppsettings("UKM1ExamYear") & "'"
        Dim strTotalPercentage As String = oCommon.getFieldValueInt(strSQL)

        If (Age <= 8) Then
            Dim strMinMarkAge8 As String = oCommon.getAppsettings("MinMarkAge8")
            If (CInt(strTotalPercentage) >= CInt(strMinMarkAge8)) Then
                Return True
            End If
        ElseIf (Age = 9) Then
            Dim strMinMarkAge9 As String = oCommon.getAppsettings("MinMarkAge9")
            If (CInt(strTotalPercentage) >= CInt(strMinMarkAge9)) Then
                Return True
            End If
        ElseIf (Age = 10) Then
            Dim strMinMarkAge10 As String = oCommon.getAppsettings("MinMarkAge10")
            If (CInt(strTotalPercentage) >= CInt(strMinMarkAge10)) Then
                Return True
            End If
        ElseIf (Age = 11) Then
            Dim strMinMarkAge11 As String = oCommon.getAppsettings("MinMarkAge11")
            If (CInt(strTotalPercentage) >= CInt(strMinMarkAge11)) Then
                Return True
            End If
        ElseIf (Age = 12) Then
            Dim strMinMarkAge12 As String = oCommon.getAppsettings("MinMarkAge12")
            If (CInt(strTotalPercentage) >= CInt(strMinMarkAge12)) Then
                Return True
            End If
        ElseIf (Age = 13) Then
            Dim strMinMarkAge13 As String = oCommon.getAppsettings("MinMarkAge13")
            If (CInt(strTotalPercentage) >= CInt(strMinMarkAge13)) Then
                Return True
            End If
        ElseIf (Age = 14) Then
            Dim strMinMarkAge14 As String = oCommon.getAppsettings("MinMarkAge14")
            If (CInt(strTotalPercentage) >= CInt(strMinMarkAge14)) Then
                Return True
            End If
        ElseIf (Age = 15) Then
            Dim strMinMarkAge15 As String = oCommon.getAppsettings("MinMarkAge15")
            If (CInt(strTotalPercentage) >= CInt(strMinMarkAge15)) Then
                Return True
            End If
        End If
        ...
        'Dim strMinMark As String = ConfigurationManager.AppSettings("MinMark")
        'strSQL = "SELECT TotalPercentage FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & oCommon.getAppsettings("UKM1ExamYear") & "'"
        'Dim strTotalPercentage As String = oCommon.getFieldValueInt(strSQL)
    ...
    Private Sub getDuration()
        ...
        strSQL = "SELECT ExamStart FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & oCommon.getAppsettings("UKM1ExamYear") & "'"
        ...
        strSQL = "SELECT ExamEnd FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & oCommon.getAppsettings("UKM1ExamYear") & "'"
    ...
    Private Function getTotalscore() As String
        strSQL = "SELECT TotalScore FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & oCommon.getAppsettings("UKM1ExamYear") & "'"
    ...
    Private Sub setUKM1DONE()
        ...
        ''--update once only
        strSQL = "SELECT Status FROM UKM1 WITH (NOLOCK) WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "'"
        If oCommon.getFieldValue(strSQL) = "DONE" Then
            Exit Sub
        End If

        ''update UKM1
        strSQL = "UPDATE UKM1 WITH (ROWLOCK) SET Status='DONE',LastPage='ukm1.permata.end.aspx?',ExamEnd='" & oCommon.getNow & "' WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "'"
        strRet = oCommon.ExecuteSQL(strSQL)
        ...
        strSQL = "UPDATE UKM1 WITH (ROWLOCK) SET ModA=(SELECT SUM(isnull(Q001,0)+isnull(Q002,0)+isnull(Q003,0)+isnull(Q004,0)+isnull(Q005,0)+isnull(Q006,0)+isnull(Q007,0)+isnull(Q008,0)+isnull(Q009,0)+isnull(Q010,0)+isnull(Q011,0)+isnull(Q012,0)+isnull(Q013,0)+isnull(Q014,0)+isnull(Q015,0)+isnull(Q016,0)+isnull(Q017,0)+isnull(Q018,0)+isnull(Q019,0)+isnull(Q020,0)+isnull(Q021,0)+isnull(Q022,0)+isnull(Q023,0)+isnull(Q024,0)+isnull(Q025,0)+isnull(Q026,0)+isnull(Q027,0)+isnull(Q028,0)+isnull(Q029,0)+isnull(Q030,0)+isnull(Q031,0)+isnull(Q032,0)+isnull(Q033,0)+isnull(Q034,0)+isnull(Q035,0)+isnull(Q036,0)+isnull(Q037,0)+isnull(Q038,0)+isnull(Q039,0)+isnull(Q040,0)+isnull(Q041,0)+isnull(Q042,0)+isnull(Q043,0)+isnull(Q044,0)+isnull(Q045,0)+isnull(Q046,0)+isnull(Q047,0)+isnull(Q048,0)+isnull(Q049,0)+isnull(Q050,0)+isnull(Q051,0)+isnull(Q052,0)+isnull(Q053,0)+isnull(Q054,0)+isnull(Q055,0)+isnull(Q056,0)+isnull(Q057,0)+isnull(Q058,0)+isnull(Q059,0)+isnull(Q060,0)) as ModA FROM UKM1 WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "') WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "'"
        oCommon.ExecuteSQL(strSQL)
        ''-debug 
        'Response.Write("ModA:" & strSQL)

        ''--SUM here ModB. TOTAL=30. Soalan 61-90
        strSQL = "UPDATE UKM1 WITH (ROWLOCK) SET ModB=(SELECT SUM(isnull(Q061,0)+isnull(Q062,0)+isnull(Q063,0)+isnull(Q064,0)+isnull(Q065,0)+isnull(Q066,0)+isnull(Q067,0)+isnull(Q068,0)+isnull(Q069,0)+isnull(Q070,0)+isnull(Q071,0)+isnull(Q072,0)+isnull(Q073,0)+isnull(Q074,0)+isnull(Q075,0)+isnull(Q076,0)+isnull(Q077,0)+isnull(Q078,0)+isnull(Q079,0)+isnull(Q080,0)+isnull(Q081,0)+isnull(Q082,0)+isnull(Q083,0)+isnull(Q084,0)+isnull(Q085,0)+isnull(Q086,0)+isnull(Q087,0)+isnull(Q088,0)+isnull(Q089,0)+isnull(Q090,0)) as ModB FROM UKM1 WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & examYear & "') WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "'"
        oCommon.ExecuteSQL(strSQL)
        'Response.Write("ModB:" & strSQL)

        ''--SUM here ModC. TOTAL=30. Soalan 105-134
        strSQL = "UPDATE UKM1 WITH (ROWLOCK) SET ModC=(SELECT SUM(isnull(Q105,0)+isnull(Q106,0)+isnull(Q107,0)+isnull(Q108,0)+isnull(Q109,0)+isnull(Q110,0)+isnull(Q111,0)+isnull(Q112,0)+isnull(Q113,0)+isnull(Q114,0)+isnull(Q115,0)+isnull(Q116,0)+isnull(Q117,0)+isnull(Q118,0)+isnull(Q119,0)+isnull(Q120,0)+isnull(Q121,0)+isnull(Q122,0)+isnull(Q123,0)+isnull(Q124,0)+isnull(Q125,0)+isnull(Q126,0)+isnull(Q127,0)+isnull(Q128,0)+isnull(Q129,0)+isnull(Q130,0)+isnull(Q131,0)+isnull(Q132,0)+isnull(Q133,0)+isnull(Q134,0)) as ModC FROM UKM1 WHERE StudentID='" & Request.QueryString("studentid") & "' AND ExamYear='" & examYear & "') WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "'"
        oCommon.ExecuteSQL(strSQL)
        'Response.Write("ModC:" & strSQL)

        ''--SUM TotalScore
        strSQL = "UPDATE UKM1 WITH (ROWLOCK) SET TotalScore=(SELECT sum(ModA+ModB+ModC) as TotalScore FROM UKM1 WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "') WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "'"
        oCommon.ExecuteSQL(strSQL)
        'Response.Write("TotalScore:" & strSQL)

        ''--SUM TotalPercentage
        strSQL = "UPDATE UKM1 WITH (ROWLOCK) SET TotalPercentage=(SELECT CONVERT(DECIMAL(5,2),100.0 * (SELECT TotalScore FROM UKM1 WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "') / " & ConfigurationManager.AppSettings("FullMark") & ") FROM UKM1  WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "') WHERE StudentID='" & studentID & "' AND ExamYear='" & examYear & "'"
        oCommon.ExecuteSQL(strSQL)
        'Response.Write("TotalPercentage:" & strSQL)

        ''copy everything to UKM1 respective ones
        Dim ukm1Table As String = Common.getUKM1Table(oCommon.getAppsettings("UKM1ExamYear"))

        If Not ukm1Table = "UKM1" Then

            strSQL = "DELETE " & ukm1Table & " WHERE StudentID = '" & Request.QueryString("studentid") & "' AND ExamYear = '" & oCommon.getAppsettings("UKM1ExamYear") & "'"
            strSQL += " INSERT INTO " & ukm1Table & " SELECT * FROM UKM1 WHERE StudentID = '" & Request.QueryString("studentid") & "' AND ExamYear = '" & oCommon.getAppsettings("UKM1ExamYear") & "'"
            'Debug.WriteLine(strSQL)
            oCommon.ExecuteSQL(strSQL)
        End If

        '--insert into security audit trail table
        oCommon.LogTrail(Request.QueryString("studentid"), oCommon.getNow, Request.UserHostAddress, Request.UserHostName, Request.Browser.Browser, "END", "NA")
    ...
    Private Sub StudentProfile_load()
        ...
        strSQL = "SELECT MYKAD,StudentFullname,DOB_Year FROM StudentProfile WHERE StudentID='" & Request.QueryString("studentid") & "'"
    ...
    Private Sub btnCreate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreate.Click
        ...
        Try
            ...
            msFileName = Request.QueryString("studentid") & "-" & lblExamYear.Text & ".pdf"
            ...
            myDocument.AddAuthor("")
            myDocument.AddSubject("Menamatkan Ujian Dalam Talian UKM1 " & oCommon.getAppsettings("UKM1ExamYear"))
