...
Public Class lecturer_counselor_Detail_Case
    ...
    Private Sub case_list()
        strSQL = "SELECT case_Name FROM case_info order by case_Name ASC"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
    ...
    Private Sub capture_data_list()
        ...
        strSQL = "select student_info.student_Name, student_info.student_ID, student_info.student_Mykad, class_info.class_Name, dicipline_info.Dicipline_Date, 
	              case_info.case_Name, staff_info.stf_ID, dicipline_info.Detail_Case, warning_letters_table.title,
    ...
    Private Sub Btnback_ServerClick(sender As Object, e As EventArgs) Handles Btnback.ServerClick
        Response.Redirect("pengajar_carian_pelajar_kaunselor.aspx?stf_ID=" + Request.QueryString("stf_ID"))
    End Sub
