...
Public Class class_Create
        ...
        Try
            If Not IsPostBack Then
                ...
                ElseIf data = "TRUE" Then
                    ...
                    classExist.visible = False

                    year_list()
                    sem_list()

                    course_Name.Enabled = False
                    staff_ID.Enabled = False
    ...
    Private Sub btnClassCreate_ServerClick(sender As Object, e As EventArgs) Handles btnClassCreate.ServerClick
        ...
        If errorCount = 1 Then
            Response.Redirect("admin_daftar_kelas.aspx?result=-1&admin_ID=" + Request.QueryString("admin_ID"))
        ElseIf errorCount = 0 Then
            Response.Redirect("admin_daftar_kelas.aspx?result=1&admin_ID=" + Request.QueryString("admin_ID"))
        ElseIf errorCount = 2 Then
            Response.Redirect("admin_daftar_kelas.aspx?result=2&admin_ID=" + Request.QueryString("admin_ID"))
        ElseIf errorCount = 3 Then
            Response.Redirect("admin_daftar_kelas.aspx?result=3&admin_ID=" + Request.QueryString("admin_ID"))
        ElseIf errorCount = 4 Then
            Response.Redirect("admin_daftar_kelas.aspx?result=4&admin_ID=" + Request.QueryString("admin_ID"))
        ElseIf errorCount = 5 Then
            Response.Redirect("admin_daftar_kelas.aspx?result=5&admin_ID=" + Request.QueryString("admin_ID"))
        End If
    End Sub
    ...
    Protected Sub class_Type_SelectedIndexChanged(sender As Object, e As EventArgs) Handles class_Type.SelectedIndexChanged
        Try
            ...
            If class_Type.SelectedValue = "Electives" Then
                ...
                staff_ID.DataSource = ds
                staff_ID.DataTextField = ""
                staff_ID.DataValueField = ""
                staff_ID.DataBind()
                staff_ID.Items.Insert(0, New ListItem("Select Staff", String.Empty))

            Else
                classExist.Visible = False
                staff_ID.Enabled = True
                course_Name.Enabled = False
                staff_info_list()
    ...
    Private Sub courseName_Load()
        strSQL = "SELECT subject_Name ,subject_ID FROM subject_info Where subject_year = '" & ddlYear.SelectedValue & "' 
                  and subject_type != 'Compulsory' and subject_StudentYear = '" & class_Level.SelectedValue & "' order by subject_Name ASC"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
    ...
    Private Sub btnEditClass_ServerClick(sender As Object, e As EventArgs) Handles btnEditClass.ServerClick
        Try
