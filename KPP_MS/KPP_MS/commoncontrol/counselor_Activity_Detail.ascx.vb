...
Public Class counselor_Activity_Detail
        ...
        Try
            If Not IsPostBack Then
                ...
                If Session("getStatus") = "VCR_CGPA" Then
                    txtbreadcrum1.Text = "View Counselor Report (Underachiever)"
                    ...
                    Table_CAD.Visible = True
                    Table_CAD_INC.Visible = False
                    Row_CAD_IT.Visible = True
                    ROW_CAD_CC.Visible = True
                    Table_CAD_INC_SUG.Visible = False
                ...
                ElseIf Session("getStatus") = "VCR_DI" Then
                    txtbreadcrum1.Text = "View Counselor Report (Discipline Issues)"
                ...
                ElseIf Session("getStatus") = "VCR_SE" Then
                    txtbreadcrum1.Text = "View Counselor Report (Social Emoitional)"
                    Exam_Show.Visible = False
                    CGPA_Show.Visible = False
                    Case_Show.Visible = False
                    Demerit_Schow.Visible = False
                ...
                ElseIf Session("getStatus") = "VCR_INC" Then
                    txtbreadcrum1.Text = "View Counselor Report (I Need Counselling)"
                    Exam_Show.Visible = False
                    CGPA_Show.Visible = False
                    Case_Show.Visible = False
                    Demerit_Schow.Visible = False
                    ...
                    SE_Status_Two.Visible = False
                    SE_Status_Three.Visible = False
                    Table_CAD.Visible = False
                    Table_CAD_INC.Visible = True
                    Row_CAD_IT.Visible = False
                    ROW_CAD_CC.Visible = False
                    Table_CAD_INC_SUG.Visible = True

                    Load_Attendance()
                    Load_NewCounselor()
                    Load_INC_NewCounselor()
                    VCR_INC_Load()
                End If
    ...
    Private Sub Checking_MenuAccess_Load()
        ...
        btn_updatecounseling_VCRCGPA.Visible = False
        btn_Exportcounseling_VCRCGPA.Visible = False
        ...
        For num As Integer = 0 To find_CountNo_LoginID - 1 Step 1
            ...
            If find_Data_SubMenu2 = "View Counselor Report" And find_Data_SubMenu2.Length > 0 Then
                ...
                If find_Data_F1Register.Length > 0 And find_Data_F1Register = "TRUE" Then
                    btn_updatecounseling_VCRCGPA.Visible = True
                End If

                If find_Data_F1PrintInBI.Length > 0 And find_Data_F1PrintInBI = "TRUE" Then
                    btn_Exportcounseling_VCRCGPA.Visible = True
                End If
            End If
    ...
    Private Sub VCR_DI_Load()

        ''Get All The Information
        strSQL = "  Select B.student_Name, D.Dicipline_Date, E.class_Name, F.case_Name, F.case_MeritDemerit_Point, A.CI_Date, A.CI_StartTime, A.CI_EndTime, A.CI_Session,
                    A.CI_Attendance, A.CI_ClienClassification, A.CI_Type, A.CI_StudentCondition, A.CI_StudentBackground, A.CI_CounselingRemark, A.CI_Action,
                    A.CI_DT_MB, A.CI_DT_MT, A.CI_DT_MDMM, A.CI_DT_MDMA, A.CI_DT_MPPM, A.CI_DT_MS, CI_SuggestionNewCounselor, CI_SuggestionNewSession, CI_SuggestionEndSession, CI_SuggestionNewSession_No, CI_SuggestionNewCounselor_StfID from counselor_info A
                    left join student_info B on A.std_ID = B.std_ID
                    left join staff_info C on A.stf_ID = C.stf_ID
                    left join dicipline_info D on A.disiplin_ID = D.disiplin_ID
                    left join class_info E on A.class_ID = E.class_ID
                    left join case_info F on D.case_ID = F.case_ID
                    where A.CI_ID = '" & Session("get_CIID") & "'"
        ...
        If MyTable.Rows.Count > 0 Then
            ...
            If Not IsDBNull(ds.Tables(0).Rows(0).Item("Dicipline_Date")) Then
                txtexamDate_VCRCGPA.Text = ds.Tables(0).Rows(0).Item("Dicipline_Date")
            Else
                txtexamDate_VCRCGPA.Text = ""
            End If
            ...
            If Not IsDBNull(ds.Tables(0).Rows(0).Item("case_Name")) Then
                txtcase_VCRDI.Text = ds.Tables(0).Rows(0).Item("case_Name")
            Else
                txtcase_VCRDI.Text = ""
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("case_MeritDemerit_Point")) Then
                txtdp_VCRDI.Text = ds.Tables(0).Rows(0).Item("case_MeritDemerit_Point")
            Else
                txtdp_VCRDI.Text = ""
            End If
            ...
            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CI_SuggestionNewSession")) Then
                CB_VCRCGPA_CallingSession.Checked = ds.Tables(0).Rows(0).Item("CI_SuggestionNewSession")
            Else
                CB_VCRCGPA_CallingSession.Checked = "False"
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CI_SuggestionNewCounselor")) Then
                CB_VCRCGPA_NewCounselor.Checked = ds.Tables(0).Rows(0).Item("CI_SuggestionNewCounselor")
            Else
                CB_VCRCGPA_NewCounselor.Checked = "False"
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CI_SuggestionEndSession")) Then
                CB_VCRCGPA_CounsellingEnd.Checked = ds.Tables(0).Rows(0).Item("CI_SuggestionEndSession")
            Else
                CB_VCRCGPA_CounsellingEnd.Checked = "False"
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CI_SuggestionNewSession_No")) Then
                txt_CallingSession.Text = ds.Tables(0).Rows(0).Item("CI_SuggestionNewSession_No")
            Else
                txt_CallingSession.Text = ""
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CI_SuggestionNewCounselor_StfID")) Then
                ddl_NewCounselor.SelectedValue = ds.Tables(0).Rows(0).Item("CI_SuggestionNewCounselor_StfID")
            Else
                ddl_NewCounselor.SelectedValue = ""
            End If
    ...
    Private Sub VCR_INC_Load()

        ''Get All The Information
        strSQL = "  Select A.CINC_ID, B.student_Name, B.student_ID, D.staff_Name, A.CINC_Session, C.class_Name, A.CINC_Date, A.CINC_StartTime, A.CINC_EndTime, A.CINC_Status, A.CINC_Status as CINC_Status_Color, A.CINC_SD_Reorder, A.CINC_Attendace, 
        ...
        If MyTable.Rows.Count > 0 Then
            ...
            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CINC_Date")) Then
                txtcounselingDate_VCRCGPA.Text = ds.Tables(0).Rows(0).Item("CINC_Date")
            Else
                txtcounselingDate_VCRCGPA.Text = ""
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CINC_StartTime")) Then
                txtstart_VCRCGPA.Text = ds.Tables(0).Rows(0).Item("CINC_StartTime")
            Else
                txtstart_VCRCGPA.Text = ""
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CINC_EndTime")) Then
                txtend_VCRCGPA.Text = ds.Tables(0).Rows(0).Item("CINC_EndTime")
            Else
                txtend_VCRCGPA.Text = ""
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CINC_Session")) Then
                txtsession_VCRCGPA.Text = ds.Tables(0).Rows(0).Item("CINC_Session")
            Else
                txtsession_VCRCGPA.Text = ""
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CINC_Attendace")) Then
                ddlattendance_VCRCGPA.SelectedValue = ds.Tables(0).Rows(0).Item("CINC_Attendace")
            Else
                ddlattendance_VCRCGPA.SelectedValue = ""
            End If
            ...
            If Not IsDBNull(ds.Tables(0).Rows(0).Item("CINC_SG_NewCounselor_stfID")) Then
                ddl_NewCounselor_NewINC.SelectedValue = ds.Tables(0).Rows(0).Item("CINC_SG_NewCounselor_stfID")
            Else
                ddl_NewCounselor_NewINC.SelectedIndex = 0
            End If
    ...
    Private Sub Load_INC_NewCounselor()
        strSQL = "  select A.stf_ID, A.staff_Name from staff_info A left join staff_Login B on A.stf_ID = B.stf_ID
                    where A.staff_Status = 'Access' and (B.staff_Access = 'KSLR' or B.staff_Access = 'KKSLR') and staff_Campus = '" & Session("SchoolCampus") & "'
                    order by staff_Name asc"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        ...
        Try
            ...
            ddl_NewCounselor_NewINC.DataTextField = "staff_Name"
            ddl_NewCounselor_NewINC.DataValueField = "stf_ID"
            ddl_NewCounselor_NewINC.DataBind()
            ddl_NewCounselor_NewINC.Items.Insert(0, New ListItem("Select Counselor Name", String.Empty))
            ddl_NewCounselor_NewINC.SelectedIndex = 0
    ...
    Private Sub btn_updatecounseling_VCRCGPA_ServerClick(sender As Object, e As EventArgs) Handles btn_updatecounseling_VCRCGPA.ServerClick
        Try
            ...
            If Session("get_CIID") <> "" And Session("getStatus") = "VCR_CGPA" Then

                Dim get_CI_DT_MB As String = "False"
                Dim get_CI_DT_MT As String = "False"
                Dim get_CI_DT_MDMM As String = "False"
                Dim get_CI_DT_MDMA As String = "False"
                Dim get_CI_DT_MPPM As String = "False"
                Dim get_CI_DT_MS As String = "False"
                Dim get_CI_SU_NS As String = "False"
                Dim get_CI_SU_NC As String = "False"
                Dim get_CI_SU_SE As String = "False"

                If CB_VCRCGPA_MB.Checked = True Then
                    get_CI_DT_MB = "True"
                End If

                If CB_VCRCGPA_MT.Checked = True Then
                    get_CI_DT_MT = "True"
                End If

                If CB_VCRCGPA_MDMM.Checked = True Then
                    get_CI_DT_MDMM = "True"
                End If

                If CB_VCRCGPA_MDMA.Checked = True Then
                    get_CI_DT_MDMA = "True"
                End If

                If CB_VCRCGPA_MPPM.Checked = True Then
                    get_CI_DT_MPPM = "True"
                End If

                If CB_VCRCGPA_MS.Checked = True Then
                    get_CI_DT_MS = "True"
                End If

                If CB_VCRCGPA_CallingSession.Checked = True Then
                    get_CI_SU_NS = "True"

                    If txt_CallingSession.Text.Length = 0 Then
                        ShowMessage(" Please Fill In Next Session ", MessageType.Error)
                        Exit Sub
                    End If
                End If

                If CB_VCRCGPA_NewCounselor.Checked = True Then
                    get_CI_SU_NC = "True"

                    If ddl_NewCounselor.SelectedIndex = 0 Then
                        ShowMessage(" Please Selct New Counselor ", MessageType.Error)
                        Exit Sub
                    End If
                End If

                If CB_VCRCGPA_CounsellingEnd.Checked = True Then
                    get_CI_SU_SE = "True"
                End If

                ''Get Time
                Dim Get_Start_Time As String = txtstart_VCRCGPA.Text.Substring(0, 5)
                Get_Start_Time = Get_Start_Time.Remove(2, 1)

                Dim Get_End_Time As String = txtend_VCRCGPA.Text.Substring(0, 5)
                Get_End_Time = Get_End_Time.Remove(2, 1)

                Dim Get_Start_Tme_Clock As String = txtstart_VCRCGPA.Text.Substring(txtstart_VCRCGPA.Text.Length - 2, 2)
                Dim Get_End_Time_Clock As String = txtend_VCRCGPA.Text.Substring(txtend_VCRCGPA.Text.Length - 2, 2)

                If Get_Start_Tme_Clock = "PM" Then
                    Get_Start_Time = Get_Start_Time + 1200
                End If

                If Get_End_Time_Clock = "PM" Then
                    Get_End_Time = Get_End_Time + 1200
                End If

                strSQL = "  update counselor_info set CI_Attendance = '" & ddlattendance_VCRCGPA.SelectedValue & "', CI_ClienClassification = '" & txtclientClassification_VCRCGPA.Text & "', CI_StudentCondition = '" & txtstudentCondition_VCRCGPA.Text & "', CI_StudentBackground = '" & txtstudentBackground_VCRCGPA.Text & "', CI_Status = 'Done',
                            CI_CounselingRemark = '" & txtremark_VCRCGPA.Text & "', CI_Action = '" & txtaction_VCRCGPA.Text & "', CI_DT_MB = '" & get_CI_DT_MB & "', CI_DT_MT = '" & get_CI_DT_MT & "', CI_DT_MDMM = '" & get_CI_DT_MDMM & "', CI_DT_MDMA = '" & get_CI_DT_MDMA & "', CI_DT_MPPM = '" & get_CI_DT_MPPM & "', CI_DT_MS = '" & get_CI_DT_MS & "',
                            CI_StartTime = '" & txtstart_VCRCGPA.Text & "', CI_EndTime = '" & txtend_VCRCGPA.Text & "', CI_StartTimeReorder = '" & Get_Start_Time & "', CI_EndTimeReorder = '" & Get_End_Time & "', CI_SuggestionNewSession = '" & get_CI_SU_NS & "', CI_SuggestionNewSession_No = '" & txt_CallingSession.Text & "', 
                            CI_SuggestionNewCounselor = '" & get_CI_SU_NC & "', CI_SuggestionNewCounselor_StfID = '" & ddl_NewCounselor.SelectedValue & "', CI_SuggestionEndSession = '" & get_CI_SU_SE & "'
                            where CI_ID = '" & Session("get_CIID") & "'"
            ...
            ElseIf Session("get_CIID") <> "" And Session("getStatus") = "VCR_INC" Then
                ...
                If CB_VCRINC_NewCounselor.Checked = True Then
                    ...
                    If ddl_NewCounselor_NewINC.SelectedIndex = 0 Then
                        ShowMessage(" Please Selct New Counselor ", MessageType.Error)
                        Exit Sub
                    End If
                End If
                ...
                strSQL = "  Update counselling_inc set CINC_Attendace = '" & ddlattendance_VCRCGPA.SelectedValue & "', CINC_Status = 'Done', CINC_MT = '" & get_CINC_MT & "', CINC_PL = '" & get_CINC_PL & "', CINC_CA = '" & get_CINC_CA & "', CINC_IA = '" & get_CINC_IA & "',
                            ...
                            CINC_StartTime = '" & txtstart_VCRCGPA.Text & "', CINC_EndTime = '" & txtend_VCRCGPA.Text & "', CINC_SD_Reorder = '" & Get_Start_Time & "', CINC_ED_Reorder = '" & Get_End_Time & "', CINC_SG_NeedNewSession = '" & get_CINC_SG_NS & "', 
                            CINC_NeedNewSession = '" & txt_CallingSession_INC.Text & "', CINC_SG_NewCounselor = '" & get_CINC_SG_NC & "', CINC_SG_NewCounselor_stfID = '" & ddl_NewCounselor_NewINC.SelectedValue & "', CINC_SG_EndSession = '" & get_CINC_SG_SE & "' 
                ...
                If CB_VCRINC_NewCounselor.Checked = True Then
                    ...
                    strSQL = "  Insert Into counseling_inc(std_ID,stf_ID,class_ID,CINC_Status,CINC_MT,CINC_PL,CINC_CA,CINC_IA,CINC_AP,CINC_TR,CINC_TM,CINC_CG,CINC_VL,CINC_ACWL,CINC_AD,CINC_AT,CINC_DA,CINC_CAL,CINC_IS,CINC_BY,CINC_AY,CINC_SS,CINC_SH,CINC_RWF,CINC_SL,CINC_HS,CINC_OC,
                                CINC_HA,CINC_EP,CINC_WD,CINC_AX,CINC_RG,CINC_DP,CINC_FGW,CINC_SD,CINC_RL,CINC_PN,CINC_PH,CINC_SCD,CINC_MF,CINC_PNM,CINC_VA,CINC_IM,CINC_ADSAI,CINC_BC,CINC_DAS,CINC_SHA,CINC_Other,CINC_Other_Details,CINC_Year)
                                Values('" & get_StdID & "','" & ddl_NewCounselor_NewINC.SelectedValue & "','" & strRet & "','Requested','" & CB_MT.Checked & "','" & CB_PL.Checked & "','" & CB_CA.Checked & "','" & CB_IA.Checked & "','" & CB_AP.Checked & "','" & CB_TR.Checked & "',
                                '" & CB_TM.Checked & "','" & CB_CG.Checked & "','" & CB_VL.Checked & "','" & CB_ACWL.Checked & "','" & CB_AD.Checked & "','" & CB_AT.Checked & "','" & CB_DA.Checked & "','" & CB_CL.Checked & "','" & CB_IS.Checked & "','" & CB_BY.Checked & "',
                                '" & CB_AY.Checked & "','" & CB_SS.Checked & "','" & CB_SH.Checked & "','" & CB_RWF.Checked & "','" & CB_SL.Checked & "','" & CB_HS.Checked & "','" & CB_OC.Checked & "','" & CB_HA.Checked & "','" & CB_EP.Checked & "','" & CB_WD.Checked & "',
                                '" & CB_AX.Checked & "','" & CB_RG.Checked & "','" & CB_DP.Checked & "','" & CB_FGW.Checked & "','" & CB_SD.Checked & "','" & CB_RL.Checked & "','" & CB_PN.Checked & "','" & CB_PH.Checked & "','" & CB_SCD.Checked & "','" & CB_MF.Checked & "',
                                '" & CB_PNM.Checked & "','" & CB_VA.Checked & "','" & CB_IM.Checked & "','" & CB_ADSAI.Checked & "','" & CB_BC.Checked & "','" & CB_DAS.Checked & "','" & CB_SHA.Checked & "','" & CB_Other.Checked & "','" & txt_CBOther.Text & "','" & Now.Year & "')"
