...
Public Class parent_studentExam
    ...
    Private Sub load_page()
        ...
        If MyTable.Rows.Count > 0 Then
            If Not IsDBNull(ds.Tables(0).Rows(0).Item("year")) Then
                ddlexam.SelectedValue = ds.Tables(0).Rows(0).Item("year")
            Else
    ...
    Private Sub Btnprint_ServerClick(sender As Object, e As EventArgs) Handles Btnprint.ServerClick
        ...
        Dim id As String = ""
        id = Request.QueryString("std_ID")
        Dim dataSTDIDFInal As String = id
        ...
        If rbtn_Malay.Checked = True Then
            ...
            tmpSQL = "Select subject_info.subject_Name,exam_result.grade from exam_result 
                                  ...
                                  left join subject_info on course.subject_ID=subject_info.subject_ID 
                                  left join exam_Info on exam_result.exam_ID=exam_Info.exam_ID 
                                  where student_info.std_ID = '" & dataSTDIDFInal & "' and exam_Info.exam_Name = '" & ddlexam.SelectedValue & "' and exam_info.exam_Year = '" & ddlyear.SelectedValue & "'
            ...
            If Confirm_Self = "on" Then
                Dim level As String = "select student_Level from student_level where std_ID = '" & dataSTDIDFInal & "' and year = '" & ddlyear.SelectedValue & "' "
            ...
            If Confirm_Cocuricullum = "on" Then
                If ddlexam.SelectedValue = "Exam 1" Or ddlexam.SelectedValue = "Exam 5" Then
                    ...
                    Dim studentData As String = "Select student_Mykad from student_info where std.ID = '" & dataSTDIDFInal & "'"
            ...
            ''print student name
            Dim stdName As String = "select student_Name from student_info where std_ID = '" & dataSTDIDFInal & "'"
            Dim dataStdName As String = getFieldValue(stdName, strConn)

            ''print student id
            Dim stdID As String = "select student_ID from student_info where std_ID = '" & dataSTDIDFInal & "'"
            Dim dataStdID As String = getFieldValue(stdID, strConn)

            ''print student mykad
            Dim stdMykad As String = "select student_Mykad from student_info where std_ID = '" & dataSTDIDFInal & "'"
            ...
            ''Calculate PNG & PNGK 
            Dim gpa As Decimal = Cal_PNG()
            Dim cgpa As Decimal = Cal_PNGK()
    ...
    Public Function Cal_PNG()

        Dim check_png_exist As String = "select png from student_Png where std_ID = '" & Request.QueryString("std_ID") & "' and exam_Name = '" & ddlexam.SelectedValue & "' and year = '" & ddlyear.SelectedValue & "'"
        ...
        If exist_png = "" Then
            ...
            Calsumgrade = "select sum(gpa * subject_info.subject_CreditHour) 
                            ...
                            left join exam_Info on exam_result.exam_ID=exam_Info.exam_ID 
                            left join subject_info on course.subject_ID=subject_info.subject_ID 
                            where student_info.std_ID = '" & Request.QueryString("std_ID") & "' and exam_Info.exam_Name = '" & ddlexam.SelectedValue & "' and exam_info.exam_Year = '" & ddlyear.SelectedValue & "'
                            and subject_info.subject_Name != 'Self Development' and  subject_info.subject_NameBM != 'Pembangunan Diri'
                            and subject_info.subject_Name not like '%Personality Development%' and  subject_info.subject_NameBM not like '%Jati Diri%'
                            and subject_info.subject_Name  not like '%Research Skill%' and  subject_info.subject_NameBM not like '%Kemahiran Penyelidikan%'
                            and subject_info.subject_Name  not like '%Portfolio%' and  subject_info.subject_NameBM not like '%Portfolio%'"
            ...
            exist = "select exam_Name from student_Png where std_ID = '" & Request.QueryString("std_ID") & "' and year = '" & ddlyear.SelectedValue & "' and exam_Name = '" & dataExmCode & "'"
            dataExist = getFieldValue(exist, strConn)

            If dataExist <> dataExmCode Then
                '' save in db
                Using PNGDATA As New SqlCommand("INSERT into student_Png(std_ID,exam_Name,year,png) values ('" & Request.QueryString("std_ID") & "','" & dataExmCode & "','" & ddlyear.SelectedValue & "','" & PNGS & "')", objConn)
                    objConn.Open()
    ...
    Public Function Cal_PNGK()

        Dim check_pngs_exist As String = "select pngs from student_Png where std_ID = '" & Request.QueryString("std_ID") & "' and exam_Name = '" & ddlexam.SelectedValue & "' and year = '" & ddlyear.SelectedValue & "'"
        Dim exist_pngs As String = getFieldValue(check_pngs_exist, strConn)

        If exist_pngs = "" Then

            '' count number of exam
            Dim CountExam As String = "Select count(exam_Name) from student_Png where std_ID = '" & Request.QueryString("std_ID") & "' "
            Dim dataCountExam As String = getFieldValue(CountExam, strConn)

            '' sum all png
            Dim SumPng As String = "select sum(png) from student_Png where std_ID = '" & Request.QueryString("std_ID") & "'"
            ...
            strSQL = "UPDATE student_Png SET pngs ='" & pngk & "' WHERE std_ID ='" & Request.QueryString("std_ID") & "' and exam_Name = '" & ddlexam.SelectedValue & "' and year = '" & ddlyear.SelectedValue & "'"
